# tlsfingerprint.com - Server-side HTTP/TLS tracking demo in Go

tlsfingerprint.com is a custom, low-level HTTP/1, HTTP/2, and HTTP/3 server that responds with fine-grained request details.

It returns JA3/JA4, Akamai H2 fingerprint, header order, H2 frames, and more.
...
```bash
$ docker build -t tlsfingerprint .
$ docker run --rm --net=host \
  --cap-add=NET_ADMIN --cap-add=NET_RAW \
  -v $PWD/config.json:/app/config.json \
  -v $PWD/certs:/app/certs \
  -v $PWD/blockedIPs:/app/blockedIPs \
  tlsfingerprint
```

If you cannot use `--net=host`, expose ports instead, but packet sniffing will only see container traffic:
...
HTTP/3 needs UDP port 443 open, and packet sniffing requires root or `NET_RAW`/`NET_ADMIN` capabilities on the host interface.

## Running it (Without Docker)

You can build a binary by running `go build -o tlsfingerprint cmd/main.go`

After that, just run the binary (`sudo ./tlsfingerprint`)
